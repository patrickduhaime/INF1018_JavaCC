...
public class JavaParser implements JavaParserConstants {
        ...
        private static ArrayList<JavaParser> parsers = new ArrayList<JavaParser>();
     ...
     */
        ...
        public static void init() {
                /*listClasses = new ArrayList<Classe>();
	  	listAttributs = new ArrayList<Attribut>();
	  	listInheritance = new ArrayList<String >();
	  	pile = new Stack<Element>();*/
        }
   	...
   	*/
        ...
        private static void parseDirectory(String path) {
                ...
                try {
                        for (int i = 0; i < listOfFiles.length; i++) {
                                 if (listOfFiles[i].isFile()&& getFileExtension(listOfFiles[i]).equals(".java"))
                                parsers.add(new JavaParser(new java.io.FileInputStream(path + "\u005c\u005c" + listOfFiles[i].getName())));
        ...
        private static void analyse() {
                ...
                //Pour chaque classe du fichier ou du document
                for(int i = 0 ; i < listClasses.size() ; i++) {
                        Classe currentClasse = listClasses.get(i);
                        ...
                        for(int j = 0 ; j < currentClasse.getMethodes().size() ; j++){
                                ...
                                for(int k = 0 ; k < currentMethode.getVariables().size() ; k++){
...
/*
		//QUESTION 1 : Visibilité
		int pubAtt = 0, proAtt = 0, priAtt = 0;
		String visi = "";
 ...
 */
  final public void Modifiers() throws ParseException {
    ...
    while (true) {
      ...
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
      case PUBLIC:
        lastModifier = jj_consume_token(PUBLIC);
        break;
      case STATIC:
        jj_consume_token(STATIC);
        break;
      case PROTECTED:
        lastModifier = jj_consume_token(PROTECTED);
        break;
      case PRIVATE:
        lastModifier = jj_consume_token(PRIVATE);
        break;
      case FINAL:
...
// Handle classes, interfaces, enums, and annotations.
  ...
  boolean isClass = false;
    ...
    lastId = jj_consume_token(IDENTIFIER);
        if(isClass) {
          Classe c = new Classe(lastId.toString());
                listClasses.add(c);
                pile.push(c);
        }
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) {
                ...
                if(!pile.empty())
                        {
                                ...
                                System.out.println("la pile est vide, ClassOrInterfaceDeclaration line 769");
    if (jj_2_4(2)) {
      jj_consume_token(SEMICOLON);
    } else {
      ;
  ...
  final public void ExtendsList() throws ParseException {
    ...
    ClassOrInterfaceType();
        listInheritance.add(lastReference.toString());
                Classe lastClasse = (Classe) pile.peek();
  ...
  final public void VariableDeclarator() throws ParseException {
    ...
    default:
      ...
      ;
                                ...
                                a.setVisibilite(lastModifier != null ? lastModifier.toString() : "protected");
                                a.setId(lastId.toString());
                                a.setType(lastType);
                                if(lastType == "reference")
                                        a.setNomType(lastReference.toString());
                        listAttributs.add(a);
                        lastModifier = null;

                                if(!pile.empty())
                                {
                                        if(pile.peek().getClass().getName() == "astMaison.Classe")
                                {
                                                Classe lastClasse = (Classe) pile.peek();
                                                lastClasse.getAttributes().add(a);
                                        } else {
                                                Methode lastMethode = (Methode) pile.peek();
                                                lastMethode.getVariables().add(a);
                                        }
                                }
  }
  ...
  final public void MethodDeclarator() throws ParseException {
    lastId = jj_consume_token(IDENTIFIER);
                Methode m = new Methode(lastId.toString());
                Classe lastClasse = (Classe) pile.peek();
                                lastClasse.getMethodes().add(m);
                pile.push(m);
    FormalParameters();
    label_14:
    while (true) {
  ...
  final public void ClassOrInterfaceType() throws ParseException {
    lastReference = jj_consume_token(IDENTIFIER);
    if (jj_2_19(2)) {
      TypeArguments();
    } else {
      ;
